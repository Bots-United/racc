...
bot_weapon_select_t valve_weapon_select[] = {
   {VALVE_WEAPON_MP5, "weapon_9mmAR", 1, 0.0, 750.0, 300.0, 2000.0,
    FALSE, 90, 10, 1, TRUE, FALSE, FALSE, FALSE, 0.0, 0.0},
   {VALVE_WEAPON_RPG, "weapon_rpg", 1, 150.0, 9999.0, 0.0, 0.0,
    TRUE, 80, 1, 1, FALSE, FALSE, FALSE, FALSE, 0.0, 0.0},
   {VALVE_WEAPON_SHOTGUN, "weapon_shotgun", 1, 0.0, 150.0, 30.0, 150.0,
    FALSE, 30, 1, 2, FALSE, FALSE, FALSE, FALSE, 0.0, 0.0},
   {VALVE_WEAPON_PYTHON, "weapon_357", 1, 0.0, 700.0, 0.0, 0.0,
    FALSE, 100, 1, 0, FALSE, FALSE, FALSE, FALSE, 0.0, 0.0},
   {VALVE_WEAPON_CROWBAR, "weapon_crowbar", 1, 0.0, 40.0, 0.0, 0.0,
    TRUE, 100, 0, 0, TRUE, FALSE, FALSE, FALSE, 0.0, 0.0},
   {VALVE_WEAPON_CROSSBOW, "weapon_crossbow", 1, 100.0, 5000.0, 0.0, 0.0,
    TRUE, 100, 1, 0, FALSE, FALSE, FALSE, FALSE, 0.0, 0.0},
   {VALVE_WEAPON_HANDGRENADE, "weapon_handgrenade", 1, 250.0, 1000.0, 0.0, 0.0,
    TRUE, 100, 1, 0, FALSE, FALSE, FALSE, FALSE, 0.0, 0.0},
   {VALVE_WEAPON_SNARK, "weapon_snark", 1, 150.0, 500.0, 0.0, 0.0,
    FALSE, 100, 1, 0, FALSE, FALSE, FALSE, FALSE, 0.0, 0.0},
   {VALVE_WEAPON_EGON, "weapon_egon", 1, 0.0, 9999.0, 0.0, 0.0,
    FALSE, 100, 1, 0, TRUE, FALSE, FALSE, FALSE, 0.0, 0.0},
   {VALVE_WEAPON_GAUSS, "weapon_gauss", 1, 0.0, 9999.0, 0.0, 9999.0,
    FALSE, 70, 1, 10, FALSE, FALSE, FALSE, TRUE, 0.0, 1.4},
   {VALVE_WEAPON_HORNETGUN, "weapon_hornetgun", 1, 30.0, 1000.0, 30.0, 1000.0,
    TRUE, 50, 1, 4, FALSE, TRUE, FALSE, FALSE, 0.0, 0.0},
   {VALVE_WEAPON_GLOCK, "weapon_9mmhandgun", 1, 0.0, 1200.0, 0.0, 1200.0,
...
void BotSwitchToBestWeapon (bot_t *pBot)
{
   ...
   while (pSelect[select_index].iId)
   {
      ...
      if ((pBot->f_camp_time > gpGlobals->time)
          && ((pSelect[select_index].iId == VALVE_WEAPON_CROWBAR)
              || (pSelect[select_index].iId == VALVE_WEAPON_GLOCK)
              || (pSelect[select_index].iId == VALVE_WEAPON_SHOTGUN)
              || (pSelect[select_index].iId == VALVE_WEAPON_HORNETGUN)
              || (pSelect[select_index].iId == VALVE_WEAPON_HANDGRENADE)
              || (pSelect[select_index].iId == VALVE_WEAPON_TRIPMINE)
              || (pSelect[select_index].iId == VALVE_WEAPON_SATCHEL)
              || (pSelect[select_index].iId == VALVE_WEAPON_SNARK)))
      {
         select_index++; // skip to next weapon
         continue;
      }
...
{
   bot_weapon_select_t *pSelect = &valve_weapon_select[0];
   bot_fire_delay_t *pDelay = &valve_fire_delay[0];
   int select_index = 0;
   bool primary_ammo_left, use_primary, use_secondary;
   ...
   // is it the RPG ?
   if (pSelect[select_index].iId == VALVE_WEAPON_RPG)
   {
   ...
   if (use_primary)
   {
      ...
      if ((pSelect[select_index].iId == VALVE_WEAPON_HANDGRENADE) && (distance < 1000))
      {
      ...
      if (!((pSelect[select_index].iId == VALVE_WEAPON_CROWBAR)
   ...
   {
      // if using hand grenade, quickly adjust to correct pitch
      if ((pSelect[select_index].iId == VALVE_WEAPON_MP5) && (distance < 2000))
      {
...
void BotShootAtEnemy (bot_t *pBot)
{
   ...
   if (f_distance > 200)
   {
      bool has_sniper_rifle = ((pBot->current_weapon.iId == VALVE_WEAPON_CROSSBOW)
                               || (pBot->current_weapon.iId == VALVE_WEAPON_PYTHON));
      ...
      if ((!pBot->b_is_fearful && !has_sniper_rifle && !BotCanFallForward (pBot, &tr))
          || (pBot->current_weapon.iId == VALVE_WEAPON_CROWBAR))
      {
