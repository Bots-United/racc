         ...
         else
         {
            ...
            if (strcmp (STRING (pent->v.classname), STRING (tr.pHit->v.classname)) == 0)
            {
                   ...
                   && (fabs (entity_origin.z - pBot->pEdict->v.origin.z) < 60))
               {
                  ...
                  if (distance < 50)
                  {
                     if (pBot->current_weapon->hardware->id != VALVE_WEAPON_CROWBAR)
                        FakeClientCommand (pBot->pEdict, "weapon_crowbar"); // select a proximity weapon
                     else
                     {
...
void BotThink (bot_t *pBot)
{
   ...
   if ((pBot->f_bot_alone_timer > 0) && (pBot->f_bot_alone_timer <= *server.time))
   {
      ...
      if (!((pBot->current_weapon->hardware->id == VALVE_WEAPON_CROWBAR)
            || (pBot->current_weapon->hardware->id == VALVE_WEAPON_9MMHANDGUN)
            || (pBot->current_weapon->hardware->id == VALVE_WEAPON_SHOTGUN)
            || (pBot->current_weapon->hardware->id == VALVE_WEAPON_HGUN)
            || (pBot->current_weapon->hardware->id == VALVE_WEAPON_HANDGRENADE)
            || (pBot->current_weapon->hardware->id == VALVE_WEAPON_TRIPMINE)
            || (pBot->current_weapon->hardware->id == VALVE_WEAPON_SATCHEL)
            || (pBot->current_weapon->hardware->id == VALVE_WEAPON_SNARK)))
...
{
   if (!IsValidPlayer (pBot->pEdict) || !IsValidPlayer (pBot->BotEars.pAskingEntity))
      return; // reliability check
