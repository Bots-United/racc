...
void DLLEXPORT GiveFnptrsToDll (enginefuncs_t* pengfuncsFromEngine, globalvars_t *pGlobals)
{
   ...
   if ((fp = fopen ("dmc/dlls/metamod.dll", "rb")) != NULL)
   {
      fclose (fp); // DLL exists, close it
      h_Library = LoadLibrary ("dmc/dlls/metamod.dll");// load Metamod METAMOD.DLL
   }
   else
      h_Library = LoadLibrary ("dmc/dlls/dmc.dll"); // load Deathmatch Classic DMC.DLL

   if (!h_Library)
      ALERT (at_error, "RACC: Fatal - MOD DLL not found");

   // load exported symbol table
   LoadSymbols ("dmc/dlls/dmc.dll");
