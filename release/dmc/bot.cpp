...
void BotCreate (void)
{
   ...
   FakeClientCommand (pBot->pEdict, "_firstspawn"); // must send this to spawn in DMC
}
...
bool BotCheckForGrenades (bot_t *pBot)
{
   ...
   {
      // check if entity is a grenade from the grenadelauncher
...
void BotCheckForItems (bot_t *pBot)
{
   ...
   while ((pent = UTIL_FindEntityInSphere (pent, pBot->pEdict->v.origin, 500)) != NULL)
   {
         ...
         else
         {
            ...
            if (strcmp (STRING (pent->v.classname), STRING (tr.pHit->v.classname)) == 0)
            {
                   ...
                   && (fabs (entity_origin.z - pBot->pEdict->v.origin.z) < 60))
               {
                  ...
                  if (distance < 50)
                  {
                     if (pBot->current_weapon->hardware->id != DMC_WEAPON_AXE)
                        BotSelectWeapon (pBot, DMC_WEAPON_AXE); // select a proximity weapon
                     else
                     {
      ...
      else if (strcmp ("trigger_teleport", STRING (pent->v.classname)) == 0) // else if teleporter
      {
         // always use the teleporter if haven't used a teleporter in at least 20 seconds...
         if (pBot->f_interact_time + 20 < *server.time)
         {
            entity_origin = VecBModelOrigin (pent);
            can_pickup = TRUE;
            pBot->f_interact_time = *server.time;
         }
      }

      else if (strcmp ("trigger_push", STRING (pent->v.classname)) == 0) // else if air wind
      {
         entity_origin = VecBModelOrigin (pent);
         can_pickup = TRUE;

         // if close enough
         if ((entity_origin - pBot->pEdict->v.origin).Length () < 40)
            pBot->BotMove.f_jump_time = *server.time; // jump
      }
      ...
      else // everything else...
      {
         ...
         if (BotCanSeeThis (pBot, entity_origin))
         {
            ...
            // check if entity is a rocket incoming
            else if (strcmp ("quake_rocket", STRING (pent->v.classname)) == 0)
            {
               TraceResult tr;
...
void BotReactToSound (bot_t *pBot, noise_t *sound)
{
   ...
   // is it a player movement sound ?
   if (strncmp ("player/pl_", sound->file_path, 10) == 0)
   {
...
void PlayClientSoundsForBots (edict_t *pPlayer)
{
   ...
   // we have to simulate their emitting in order for the bots to hear them. The ladder check
   // assumes that a player moving vertically, not on the ground, having a ladder in his
   // immediate surroundings is climbing it, and the ladder sound is emitted periodically.
