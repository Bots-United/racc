...
void BotCheckForItems (bot_t *pBot)
{
   ...
   while ((pent = UTIL_FindEntityInSphere (pent, pBot->pEdict->v.origin, 500)) != NULL)
   {
         ...
         else
         {
            ...
            if (strcmp (STRING (pent->v.classname), STRING (tr.pHit->v.classname)) == 0)
            {
                        ...
                        && !(pent->v.flags & FL_WORLDBRUSH) && (abs (vecEnd.z - vecStart.z) < 60))
               {
                  ...
                  if (distance < 50)
                  {
                     if (pBot->current_weapon.iId != DMC_WEAPON_AXE)
                        BotSelectItem (pBot, DMC_WEAPON_AXE); // select a proximity weapon
                     else
                     {
      ...
      else if (strcmp ("trigger_teleport", STRING (pent->v.classname)) == 0) // else if teleporter
      {
         ...
         if (pBot->f_interact_time + 20 < gpGlobals->time)
         {
            entity_origin = VecBModelOrigin (pent);
            can_pickup = TRUE;
            pBot->f_interact_time = gpGlobals->time;
         }
      }
      ...
      else if (strcmp ("trigger_push", STRING (pent->v.classname)) == 0) // else if air wind
      {
         ...
         if ((entity_origin - pBot->pEdict->v.origin).Length () < 40)
            pBot->BotMove.f_jump_time = gpGlobals->time; // jump
      }
...
void UpdateBulletSounds (edict_t *pEdict)
{
   ...
   if ((strcmp (STRING (pEdict->v.weaponmodel), "models/p_crowbar") == 0)
       || (strcmp (STRING (pEdict->v.weaponmodel), "models/p_rock") == 0)
       || (strcmp (STRING (pEdict->v.weaponmodel), "models/p_rock2") == 0))
