   ...
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      ...
      if (IsValidPlayer (ed))
      {
         ...
         if (ed->v.flags & FL_THIRDPARTYBOT)
         {
            ...
            if (msg_type == GetUserMsgId ("WeaponList"))
               botMsgFunction = BotClient_ASheep_WeaponList;
            else if (msg_type == GetUserMsgId ("CurWeapon"))
               botMsgFunction = BotClient_ASheep_CurWeapon;
            else if (msg_type == GetUserMsgId ("AmmoX"))
               botMsgFunction = BotClient_ASheep_AmmoX;
            else if (msg_type == GetUserMsgId ("AmmoPickup"))
               botMsgFunction = BotClient_ASheep_AmmoPickup;
            else if (msg_type == GetUserMsgId ("WeapPickup"))
               botMsgFunction = BotClient_ASheep_WeapPickup;
            else if (msg_type == GetUserMsgId ("ItemPickup"))
               botMsgFunction = BotClient_ASheep_ItemPickup;
            else if (msg_type == GetUserMsgId ("Health"))
               botMsgFunction = BotClient_ASheep_Health;
            else if (msg_type == GetUserMsgId ("Damage"))
               botMsgFunction = BotClient_ASheep_Damage;
            else if (msg_type == GetUserMsgId ("ScreenFade"))
               botMsgFunction = BotClient_ASheep_ScreenFade;
            else if (msg_type == GetUserMsgId ("SayText"))
               botMsgFunction = BotClient_ASheep_SayText;
         }
      }
      ...
      else if (msg_dest == MSG_ALL)
      {
         ...
         if (msg_type == GetUserMsgId ("DeathMsg"))
            botMsgFunction = BotClient_ASheep_DeathMsg_All;
      }
   }
...
int pfnIndexOfEdict (const edict_t *pEdict)
{
   ...
   // pEdict. It is the actual engine function that is called behind the ENTINDEX() macro.
