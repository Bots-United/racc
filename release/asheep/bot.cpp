...
bool BotCheckForGrenades (bot_t *pBot)
{
   ...
   while ((pGrenade = UTIL_FindEntityInSphere (pGrenade, pBot->pEdict->v.origin, 400)) != NULL)
   {
      ...
      if (((strcmp ("grenade", STRING (pGrenade->v.classname)) == 0)
           ...
           || (strcmp ("monster_toad", STRING (pGrenade->v.classname)) == 0)
          ...
          && (IsInPlayerFOV (pBot->pEdict, pGrenade->v.origin)))
      {
         ...
         // if this grenade is a snark or a toad...
         ...
         else
         {
            ...
            if (strcmp (STRING (pent->v.classname), STRING (tr.pHit->v.classname)) == 0)
            {
                   ...
                   && (fabs (entity_origin.z - pBot->pEdict->v.origin.z) < 60))
               {
                  ...
                  if (distance < 50)
                  {
                     if ((pBot->current_weapon->hardware->id != ASHEEP_WEAPON_IRONBAR)
                         && (pBot->current_weapon->hardware->id != ASHEEP_WEAPON_POOLSTICK))
                     {
                        FakeClientCommand (pBot->pEdict, "weapon_poolstick"); // select a proximity weapon...
                        FakeClientCommand (pBot->pEdict, "weapon_crowbar"); // with preference for the iron bar
                     }
                     else
                     {
...
void BotPreThink (bot_t *pBot)
{
   ...
   return; // finished bot's sensing part of the AI cycle
}
...
void BotThink (bot_t *pBot)
{
   ...
   if ((pBot->f_bot_alone_timer > 0) && (pBot->f_bot_alone_timer <= *server.time))
   {
      ...
      if (!((pBot->current_weapon->hardware->id == ASHEEP_WEAPON_IRONBAR)
            || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_GLOCK)
            || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_SHOTGUN)
            || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_HORNETGUN)
            || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_HANDGRENADE)
            || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_TRIPMINE)
            || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_SATCHEL)
            || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_SNARK)
            || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_TOAD)
            || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_POOLSTICK)))
