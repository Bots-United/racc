         ...
         else
         {
            ...
            if (strcmp (STRING (pent->v.classname), STRING (tr.pHit->v.classname)) == 0)
            {
                   ...
                   && !(pent->v.flags & FL_WORLDBRUSH) && (abs (vecEnd.z - vecStart.z) < 60))
               {
                  ...
                  if (distance < 50)
                  {
                     if ((pBot->current_weapon.iId != ASHEEP_WEAPON_IRONBAR) && (pBot->current_weapon.iId != ASHEEP_WEAPON_POOLSTICK))
                     {
                        BotSelectItem (pBot, "weapon_poolstick"); // select a proximity weapon...
                        BotSelectItem (pBot, "weapon_crowbar"); // with preference for the iron bar
                     }
                     else
                     {
...
void BotThink (bot_t *pBot)
{
   ...
   pBot->BotMove.f_move_speed = 0.0; // reset move_speed
   ...
   if ((pBot->f_bot_alone_timer > 0) && (pBot->f_bot_alone_timer <= gpGlobals->time))
   {
      ...
      if (!((pBot->current_weapon.iId == ASHEEP_WEAPON_IRONBAR)
            || (pBot->current_weapon.iId == ASHEEP_WEAPON_GLOCK)
            || (pBot->current_weapon.iId == ASHEEP_WEAPON_SHOTGUN)
            || (pBot->current_weapon.iId == ASHEEP_WEAPON_HORNETGUN)
            || (pBot->current_weapon.iId == ASHEEP_WEAPON_HANDGRENADE)
            || (pBot->current_weapon.iId == ASHEEP_WEAPON_TRIPMINE)
            || (pBot->current_weapon.iId == ASHEEP_WEAPON_SATCHEL)
            || (pBot->current_weapon.iId == ASHEEP_WEAPON_SNARK)
            || (pBot->current_weapon.iId == ASHEEP_WEAPON_TOAD)
...
void UpdateBulletSounds (edict_t *pEdict)
{
   ...
   if ((strcmp (STRING (pEdict->v.weaponmodel), "models/p_crowbar") == 0)
       || (strcmp (STRING (pEdict->v.weaponmodel), "models/p_poolstick") == 0)
       ...
       || (strcmp (STRING (pEdict->v.weaponmodel), "models/p_kmedkit") == 0)
       ...
       || (strcmp (STRING (pEdict->v.weaponmodel), "models/p_toad") == 0)
