...
edict_t *BotCheckForEnemies (bot_t *pBot)
{
   ...
   else if (((pBot->current_weapon->hardware->id == ASHEEP_WEAPON_CROSSBOW)
             || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_PYTHON))
...
void BotSwitchToBestWeapon (bot_t *pBot)
{
   ...
   for (weapon_index = 0; weapon_index < weapon_count; weapon_index++)
   {
      ...
      if ((pBot->f_camp_time > *server.time)
          && ((weapons[weapon_index].id == ASHEEP_WEAPON_IRONBAR)
              || (weapons[weapon_index].id == ASHEEP_WEAPON_GLOCK)
              || (weapons[weapon_index].id == ASHEEP_WEAPON_SHOTGUN)
              || (weapons[weapon_index].id == ASHEEP_WEAPON_HORNETGUN)
              || (weapons[weapon_index].id == ASHEEP_WEAPON_HANDGRENADE)
              || (weapons[weapon_index].id == ASHEEP_WEAPON_TRIPMINE)
              || (weapons[weapon_index].id == ASHEEP_WEAPON_SATCHEL)
              || (weapons[weapon_index].id == ASHEEP_WEAPON_SNARK)
              || (weapons[weapon_index].id == ASHEEP_WEAPON_TOAD)
              || (weapons[weapon_index].id == ASHEEP_WEAPON_POOLSTICK)))
         continue; // skip to next weapon
...
void BotFireWeapon (bot_t *pBot, Vector v_enemy, int weapon_choice)
{
   ...
   if (weapon_index == weapon_count)
   {
      for (weapon_index = 0; weapon_index < weapon_count; weapon_index++)
      {
         ...
         if (use_primary && use_secondary)
         {
            if (primary_percent <= weapons[weapon_index].secondary.use_percent)
               use_secondary = FALSE; // choose first of both from use percents
            else
               use_primary = FALSE; // choose second of both from use percents
         }
   ...
   if ((weapons[weapon_index].id == ASHEEP_WEAPON_CROSSBOW)
       || (weapons[weapon_index].id == ASHEEP_WEAPON_PYTHON))
   {
      ...
      if (!(tr.pHit->v.flags & (FL_MONSTER | FL_CLIENT))
          && (RANDOM_LONG (1, 100) < 94 + pBot->pProfile->skill))
   ...
   else if (weapons[weapon_index].id == ASHEEP_WEAPON_HANDGRENADE)
   {
   ...
   // else is it the RPG ?
   else if (weapons[weapon_index].id == ASHEEP_WEAPON_RPG)
   {
      ...
      // it is safe to throw a rocket here, so FIRE !!
      pBot->pEdict->v.button |= IN_ATTACK;
   }

   // else is it a melee weapon ?
   else if ((weapons[weapon_index].id == ASHEEP_WEAPON_IRONBAR)
            || (weapons[weapon_index].id == ASHEEP_WEAPON_POOLSTICK))
   {
...
void BotShootAtEnemy (bot_t *pBot)
{
   ...
   has_sniper_rifle = ((pBot->current_weapon->hardware->id == ASHEEP_WEAPON_CROSSBOW)
                       || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_PYTHON));
   has_proximity_weapon = ((pBot->current_weapon->hardware->id == ASHEEP_WEAPON_IRONBAR)
                           || (pBot->current_weapon->hardware->id == ASHEEP_WEAPON_POOLSTICK));
   has_grenade = ((pBot->current_weapon->hardware->id == ASHEEP_WEAPON_HANDGRENADE)
