...
BOOL WINAPI DllMain (HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
   ...
   return TRUE; // required DLL entry point
}
...
void DLLEXPORT GiveFnptrsToDll (enginefuncs_t* pengfuncsFromEngine, globalvars_t *pGlobals)
{
   ...
   if ((fp = fopen ("asheep/dlls/metamod.dll", "rb")) != NULL)
   {
      fclose (fp); // DLL exists, close it
      h_Library = LoadLibrary ("asheep/dlls/metamod.dll");// load Metamod METAMOD.DLL
   }
   else
      h_Library = LoadLibrary ("asheep/dlls/hl.dll"); // load Azure Sheep HL.DLL

   if (!h_Library)
      ALERT (at_error, "RACC: Fatal - MOD DLL not found");

   // load exported symbol table
   LoadSymbols ("asheep/dlls/hl.dll");
...
int DispatchSpawn (edict_t *pent)
{
   ...
   if (is_multiplayer_game)
   {
      ...
      // else if we are spawning a HEV charger or a battery
      else if ((strcmp (STRING (pent->v.classname), "item_battery") == 0)
          || (strcmp (STRING (pent->v.classname), "func_recharge") == 0))
         return 0; // don't allow engine to spawn it (no use in Azure Sheep's multiplayer mode)
   }
   ...
   if (is_multiplayer_game)
   {
      ...
      if (bot_check_time < gpGlobals->time)
      {
         ...
         if (welcome_sent)
         {
            ...
            {
               BotCreate (NULL, NULL, NULL, -1, -1); // add a bot
...
int GetTeam (edict_t *pEntity)
{
   ...
   for (int i = 0; i < num_teams; i++)
      if (strcmp (g_engfuncs.pfnInfoKeyValue ((*g_engfuncs.pfnGetInfoKeyBuffer) (pEntity), "model"), team_names[i]) == 0)
...
void ServerCommand (char *command)
{
   ...
   if ((FStrEq (pcmd, "help")) || (FStrEq (pcmd, "?")))
   {
      ...
      UTIL_ServerConsole_printf ("racc add [name skin logo nationality skill] - Add a bot to the current game\n");
   ...
   else if (FStrEq (pcmd, "add"))
   {
      if (welcome_sent)
      {
         BotCreate (GetArg (command, 1), GetArg (command, 2), GetArg (command, 3), atoi (GetArg (command, 4)), atoi (GetArg (command, 5)));
