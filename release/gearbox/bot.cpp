...
void BotCreate (const char *name, const char *skin, const char *logo, int nationality, int skill, int team)
{
   ...
   if ((name != NULL) && (*name != 0))
   {
      ...
      if ((skin != NULL) && (*skin != 0) && !is_opfor_ctf)
   ...
   if (!FNullEnt (pBotEdict = CREATE_FAKE_CLIENT (c_name)))
   {
      ...
      pBot->bot_team = i_team; // save his team (if any)
      pBot->bot_class = -1; // use auto assign for class
      pBot->menu_notify = MSG_OPFOR_IDLE; // not selecting team yet
         ...
         else
         {
            ...
            if (strcmp (STRING (pent->v.classname), STRING (tr.pHit->v.classname)) == 0)
            {
                   ...
                   && !(pent->v.flags & FL_WORLDBRUSH) && (abs (vecEnd.z - vecStart.z) < 60))
               {
                  ...
                  if (distance < 50)
                  {
                     if ((pBot->current_weapon.iId != GEARBOX_WEAPON_CROWBAR)
                         ...
                         || (pBot->current_weapon.iId != GEARBOX_WEAPON_PIPEWRENCH))
                     {
                        ...
                        if (RANDOM_FLOAT (1, 100) < 33)
                           BotSelectItem (pBot, "weapon_crowbar");
                        else if (RANDOM_FLOAT (1, 100) < 50)
                           BotSelectItem (pBot, "weapon_knife");
                        else
                           BotSelectItem (pBot, "weapon_pipewrench");
                     }
                     else
                     {
...
void BotThink (bot_t *pBot)
{
   ...
   if ((pBot->f_bot_alone_timer > 0) && (pBot->f_bot_alone_timer <= gpGlobals->time))
   {
      ...
      if (!((pBot->current_weapon.iId == GEARBOX_WEAPON_CROWBAR)
            || (pBot->current_weapon.iId == GEARBOX_WEAPON_GLOCK)
            || (pBot->current_weapon.iId == GEARBOX_WEAPON_SHOTGUN)
            || (pBot->current_weapon.iId == GEARBOX_WEAPON_HORNETGUN)
            || (pBot->current_weapon.iId == GEARBOX_WEAPON_HANDGRENADE)
            || (pBot->current_weapon.iId == GEARBOX_WEAPON_TRIPMINE)
            || (pBot->current_weapon.iId == GEARBOX_WEAPON_SATCHEL)
            || (pBot->current_weapon.iId == GEARBOX_WEAPON_SNARK)
            || (pBot->current_weapon.iId == GEARBOX_WEAPON_GRAPPLE)
            || (pBot->current_weapon.iId == GEARBOX_WEAPON_PIPEWRENCH)
            || (pBot->current_weapon.iId == GEARBOX_WEAPON_DISPLACER)
            || (pBot->current_weapon.iId == GEARBOX_WEAPON_KNIFE)))
      ...
      for (ind = 1; ind <= gpGlobals->maxClients; ind++)
      {
         ...
         if (IsAlive (pPlayer) && ((pPlayer->v.flags & FL_CLIENT) || (pPlayer->v.flags & FL_FAKECLIENT)))
         {
            ...
            if (BotCheckForSounds (pBot, pPlayer))
               pBot->f_sound_update_time = gpGlobals->time + 30.0; // next check in 30 seconds
         }
      }
   }
...
void UpdateBulletSounds (edict_t *pEdict)
{
   ...
   if ((strcmp (STRING (pEdict->v.weaponmodel), "models/p_bgrap") == 0)
       || (strcmp (STRING (pEdict->v.weaponmodel), "models/p_crowbar") == 0)
       || (strcmp (STRING (pEdict->v.weaponmodel), "models/p_displacer") == 0)
       ...
       || (strcmp (STRING (pEdict->v.weaponmodel), "models/p_penguin") == 0)
       ...
       || (strcmp (STRING (pEdict->v.weaponmodel), "models/p_shock") == 0)
       || (strcmp (STRING (pEdict->v.weaponmodel), "models/p_sporelauncher") == 0)
