...
void BotCreate (void)
{
   ...
   // if bot is forced to choose a team, set the forced model, else set its own in its infobuffer
   if (!is_opfor_ctf && (server.bot_forced_team[0] != 0))
      (*g_engfuncs.pfnSetClientKeyValue) (ENTINDEX (pBot->pEdict), (*g_engfuncs.pfnGetInfoKeyBuffer) (pBot->pEdict), "model", server.bot_forced_team);
   else
      (*g_engfuncs.pfnSetClientKeyValue) (ENTINDEX (pBot->pEdict), (*g_engfuncs.pfnGetInfoKeyBuffer) (pBot->pEdict), "model", pBot->pProfile->skin);
   ...
   pBot->BotEyes.BotHUD.menu_state = MENU_GEARBOX_IDLE; // not selecting team yet
   ...
   pBot->b_not_started = TRUE; // tells bot to go and select team and class if needed
}
          ...
          && (BotCanSeeOfEntity (pBot, pSpecialZone) != g_vecZero))
      {
         ...
         if ((pBot->pEdict->v.health < 100) && (pSpecialZone->v.frame == 0))
         {
            ...
            if ((VecBModelOrigin (pSpecialZone) - pBot->pEdict->v.origin).Length () < 40)
            {
               ...
               pBot->bot_task |= BOT_TASK_USINGCHARGER;
               pBot->f_use_station_time = *server.time; // remember when we started using station
            }
   ...
   while ((pGrenade = UTIL_FindEntityInSphere (pGrenade, pBot->pEdict->v.origin, 400)) != NULL)
   {
          ...
          && (IsInPlayerFOV (pBot->pEdict, pGrenade->v.origin)))
      {
         ...
         if (strcmp ("monster_snark", STRING (pGrenade->v.classname)) == 0)
            pBot->BotEnemy.pEdict = pGrenade; // bot has to get rid of this threat quickly !
         ...
         // else if this grenade is a tripmine and bot is far enough to shoot it...
            ...
            pBot->BotEnemy.pEdict = pGrenade; // bot can now get rid of this threat
         ...
         else
         {
            ...
            if (strcmp (STRING (pent->v.classname), STRING (tr.pHit->v.classname)) == 0)
            {
                   ...
                   && (fabs (entity_origin.z - pBot->pEdict->v.origin.z) < 60))
               {
                  ...
                  if (distance < 50)
                  {
                     if ((pBot->current_weapon->hardware->id != GEARBOX_WEAPON_CROWBAR)
                         || (pBot->current_weapon->hardware->id != GEARBOX_WEAPON_KNIFE)
                         || (pBot->current_weapon->hardware->id != GEARBOX_WEAPON_PIPEWRENCH))
                     {
                        // select a proximity weapon
                        if (RANDOM_FLOAT (1, 100) < 33)
                           FakeClientCommand (pBot->pEdict, "weapon_crowbar");
                        else if (RANDOM_FLOAT (1, 100) < 50)
                           FakeClientCommand (pBot->pEdict, "weapon_knife");
                        else
                           FakeClientCommand (pBot->pEdict, "weapon_pipewrench");
                     }
                     else
                     {
         ...
         if (BotCanSeeThis (pBot, entity_origin))
         {
            ...
            else if (strcmp ("beam", STRING (pent->v.classname)) == 0)
            {
               ...
               if (BotCanSeeThis (pBot, v_tripmine))
                  BotSetIdealAngles (pBot, UTIL_VecToAngles (v_tripmine - GetGunPosition (pBot->pEdict)));
            }
...
void BotThink (bot_t *pBot)
{
   ...
   if ((pBot->f_bot_alone_timer > 0) && (pBot->f_bot_alone_timer <= *server.time))
   {
      ...
      if (!((pBot->current_weapon->hardware->id == GEARBOX_WEAPON_CROWBAR)
            || (pBot->current_weapon->hardware->id == GEARBOX_WEAPON_GLOCK)
            || (pBot->current_weapon->hardware->id == GEARBOX_WEAPON_SHOTGUN)
            || (pBot->current_weapon->hardware->id == GEARBOX_WEAPON_HORNETGUN)
            || (pBot->current_weapon->hardware->id == GEARBOX_WEAPON_HANDGRENADE)
            || (pBot->current_weapon->hardware->id == GEARBOX_WEAPON_TRIPMINE)
            || (pBot->current_weapon->hardware->id == GEARBOX_WEAPON_SATCHEL)
            || (pBot->current_weapon->hardware->id == GEARBOX_WEAPON_SNARK)
            || (pBot->current_weapon->hardware->id == GEARBOX_WEAPON_GRAPPLE)
            || (pBot->current_weapon->hardware->id == GEARBOX_WEAPON_PIPEWRENCH)
            || (pBot->current_weapon->hardware->id == GEARBOX_WEAPON_DISPLACER)
            || (pBot->current_weapon->hardware->id == GEARBOX_WEAPON_KNIFE)))
   ...
   if (pBot->bot_task & BOT_TASK_USINGCHARGER)
   {
      ...
      while ((pStation = UTIL_FindEntityInSphere (pStation, pBot->pEdict->v.origin, 40)) != NULL)
      {
         if ((strcmp ("func_healthcharger", STRING (pStation->v.classname)) == 0)
            ...
            pBot->bot_task &= ~BOT_TASK_USINGCHARGER; // stop using station if healed or no power left
         else if ((strcmp ("func_recharge", STRING (pStation->v.classname)) == 0)
                  && ((pBot->pEdict->v.armorvalue == 100) || (pStation->v.frame != 0)))
            pBot->bot_task &= ~BOT_TASK_USINGCHARGER; // stop using station if healed or no power left
      }
      if (BotCheckForEnemies (pBot) != 0)
         pBot->bot_task &= ~BOT_TASK_USINGCHARGER; // stop using station if enemy found
      if (pBot->f_use_station_time + 10 < *server.time)
         pBot->bot_task &= ~BOT_TASK_USINGCHARGER; // bot is stuck trying to use station
      ...
      else if (player_velocity > MAX_WALK_SPEED)
      {
         ...
         switch (texture_type)
         {
            ...
            case CHAR_TEX_COMPUTER:
               ...
               break;
            case 'O':
               sprintf (sound_path, "player/pl_snow%d.wav", RANDOM_LONG (1, 4)); // 4 snow sounds
               volume = 0.8;
               break;
         }
