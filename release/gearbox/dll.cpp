...
int Spawn (edict_t *pent)
{
   ...
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      ...
      else if (strncmp (STRING (pent->v.classname), "item_ctf", 8) == 0)
         is_opfor_ctf = TRUE; // set CTF map notification flag if it's a flag that is spawning
...
int GetTeam (edict_t *pEntity)
{
   ...
   char player_model[32];

   // retrieve the entity's model
   strcpy (player_model, (g_engfuncs.pfnInfoKeyValue ((*g_engfuncs.pfnGetInfoKeyBuffer) (pEntity), "model")));

   // were we unable to get the model from the infobuffer ?
   if (player_model[0] == 0)
      return (-1); // entity is not a player then, we can't know its team

   // is this an Opposing Force CTF map ?
   if (is_opfor_ctf)
   {
      if ((strcmp (player_model, "ctf_barney") == 0)
          || (strcmp (player_model, "cl_suit") == 0)
          || (strcmp (player_model, "ctf_gina") == 0)
          || (strcmp (player_model, "ctf_gordon") == 0)
          || (strcmp (player_model, "otis") == 0)
          || (strcmp (player_model, "ctf_scientist") == 0))
         return (0); // entity is with the Scientists
      else if ((strcmp (player_model, "beret") == 0)
               || (strcmp (player_model, "drill") == 0)
               || (strcmp (player_model, "grunt") == 0)
               || (strcmp (player_model, "recruit") == 0)
               || (strcmp (player_model, "shephard") == 0)
               || (strcmp (player_model, "tower") == 0))
         return (1); // entity is with the Grunts
   }

   // else must be team deathmatch...
   else
   {
