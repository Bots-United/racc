...
bool is_opfor_ctf = FALSE;
char team_names[32][16];
int num_teams = 0;
int team_allies[4];
...
void DLLEXPORT GiveFnptrsToDll (enginefuncs_t* pengfuncsFromEngine, globalvars_t *pGlobals)
{
   ...
   if ((fp = fopen ("gearbox/dlls/metamod.dll", "rb")) != NULL)
   {
      fclose (fp); // DLL exists, close it
      h_Library = LoadLibrary ("gearbox/dlls/metamod.dll");// load Metamod METAMOD.DLL
   }
   else
      h_Library = LoadLibrary ("gearbox/dlls/opfor.dll"); // load Opposing Force OPFOR.DLL

   if (!h_Library)
      ALERT (at_error, "RACC: Fatal - MOD DLL not found");

   // load exported symbol table
   LoadSymbols ("gearbox/dlls/opfor.dll");
   ...
   if (is_multiplayer_game)
   {
      ...
      if (strcmp (STRING (pent->v.classname), "worldspawn") == 0)
      {
         ...
         if (is_dedicated_server)
            bot_check_time = gpGlobals->time + 40.0; // wait longer on dedicated servers
         else
            bot_check_time = gpGlobals->time + 15.0; // else wait 15 seconds before adding bots
      }

      // else if we are spawning a flag then we know it is a CTF map
      else if (strcmp (STRING (pent->v.classname), "item_ctfflag") == 0)
         is_opfor_ctf = TRUE; // set CTF map notification flag
   }
   ...
   if (is_multiplayer_game)
   {
      ...
      if (bot_check_time < gpGlobals->time)
      {
         ...
         if (welcome_sent)
         {
            ...
            {
               BotCreate (NULL, NULL, NULL, -1, -1, -1); // add a bot
...
int GetTeam (edict_t *pEntity)
{
   ...
   strcpy (model_name, (g_engfuncs.pfnInfoKeyValue ((*g_engfuncs.pfnGetInfoKeyBuffer) (pEntity), "model")));

   // is this an OpFor CTF map ?
   if (is_opfor_ctf)
   {
      if ((strcmp (model_name, "ctf_barney") == 0)
         ...
         return 0; // entity is with the Scientists
         ...
         return 1; // entity is with the Grunts
   }

   // else must be OpFor deathmatch...
   else
   {
      // compare the model name to each team name in the array...
      for (int i = 0; i < num_teams; i++)
         if (strcmp (model_name, team_names[i]) == 0)
...
void ServerCommand (char *command)
{
   ...
   if ((FStrEq (pcmd, "help")) || (FStrEq (pcmd, "?")))
   {
      ...
      UTIL_ServerConsole_printf ("racc add [name skin logo nationality skill team] - Add a bot to the current game\n");
   ...
   else if (FStrEq (pcmd, "add"))
   {
      if (welcome_sent)
      {
         BotCreate (GetArg (command, 1), GetArg (command, 2), GetArg (command, 3), atoi (GetArg (command, 4)), atoi (GetArg (command, 5)), atoi (GetArg (command, 6)));
