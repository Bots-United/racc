...
// This message is sent when the Gearbox OpFor VGUI menu is displayed.
void BotClient_Gearbox_VGUI (void *p, int bot_index)
{
   if ((*(int *) p) == 2) // is it a team selection menu?
      bots[bot_index].menu_notify = MSG_OPFOR_TEAMSELECT_TEAMMENU;

   else if ((*(int *) p) == 3) // is is a class selection menu?
      bots[bot_index].menu_notify = MSG_OPFOR_TEAMSELECT_CLASSMENU;
}
