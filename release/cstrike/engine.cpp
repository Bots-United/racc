...
edict_t *pfnFindEntityByString (edict_t *pEdictStartSearchAfter, const char *pszField, const char *pszValue)
{
   ...
   if (server.is_multiplayer && !DebugLevel.is_paused)
      if (strcmp (pszValue, "info_map_parameters") == 0)
         round.has_finished = TRUE; // catch new round message (map resetting message)
   ...
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      ...
      else if (msg_dest == MSG_ALL)
      {
         ...
         if (msg_type == GetUserMsgId ("DeathMsg"))
            botMsgFunction = BotClient_CS_DeathMsg_All;
         else if (msg_type == GetUserMsgId ("TextMsg"))
            botMsgFunction = BotClient_CS_TextMsg_All;
      }
   }
...
{
   if (hook_this) LogToFile ("MessageEnd\n");
   // only process bots if we are in multiplayer mode
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
...
{
   if (hook_this) LogToFile ("WriteByte %d\n", iValue);
   // only process bots if we are in multiplayer mode
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
...
{
   if (hook_this) LogToFile ("WriteChar %d\n", iValue);
   // only process bots if we are in multiplayer mode
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      // if this message is for a bot, call the client message function...
...
{
   if (hook_this) LogToFile ("WriteShort %d\n", iValue);
   // only process bots if we are in multiplayer mode
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      // if this message is for a bot, call the client message function...
...
{
   if (hook_this) LogToFile ("WriteLong %d\n", iValue);
   // only process bots if we are in multiplayer mode
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      // if this message is for a bot, call the client message function...
...
{
   if (hook_this) LogToFile ("WriteAngle %f\n", flValue);
   // only process bots if we are in multiplayer mode
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      // if this message is for a bot, call the client message function...
...
{
   if (hook_this) LogToFile ("WriteCoord %f\n", flValue);
   // only process bots if we are in multiplayer mode
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      // if this message is for a bot, call the client message function...
...
{
   if (hook_this) LogToFile ("WriteString %s\n", sz);
   // only process bots if we are in multiplayer mode
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      // if this message is for a bot, call the client message function...
...
{
   if (hook_this) LogToFile ("WriteEntity %d\n", iValue);
   // only process bots if we are in multiplayer mode
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      // if this message is for a bot, call the client message function...
