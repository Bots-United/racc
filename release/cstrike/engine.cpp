...
void pfnChangeLevel (char* s1, char* s2)
{
   ...
   {
      b_bomb_map = FALSE; // reset bomb map flag
...
edict_t* pfnFindEntityByString (edict_t *pEdictStartSearchAfter, const char *pszField, const char *pszValue)
{
   ...
   if (is_multiplayer_game)
      if (strcmp (pszValue, "info_map_parameters") == 0)
         roundend = TRUE; // catch new round message (map resetting message)
...
void pfnSetClientMaxspeed (const edict_t *pEdict, float fNewMaxspeed)
{
   ...
   // is this message for a bot ?
   if (index != -1)
      bots[index].BotMove.f_max_speed = fNewMaxspeed; // update this bot's max speed
