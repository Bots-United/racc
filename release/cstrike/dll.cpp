...
float f_radiotime;
float f_team_radiotime[2] = {0, 0};
bool roundend = FALSE;
bool b_bomb_map = FALSE;
bool b_bomb_planted = FALSE;
float f_bomb_explode_time = 0;
...
void DLLEXPORT GiveFnptrsToDll (enginefuncs_t* pengfuncsFromEngine, globalvars_t *pGlobals)
{
   ...
   LoadSymbols ("cstrike/dlls/mp.dll");
...
int DispatchSpawn (edict_t *pent)
{
   ...
   if (is_multiplayer_game)
   {
      ...
      // else if we are spawning a bomb target...
      ...
      if (pEntity == listenserver_edict)
      {
         ...
         else if (FStrEq (pcmd, "tracehull"))
         {
            if ((arg1 != NULL) && (*arg1 != 0))
            {
               ...
               {
                  TraceResult tr;
                  edict_t *pContainingEntity = NULL;
                  UTIL_MakeVectors (pEntity->v.v_angle); // build base vectors

                  sprintf (msg, "Tracing forward from player's view angles (%.1f %.1f %.1f)...\n", pEntity->v.v_angle.x, pEntity->v.v_angle.y, pEntity->v.v_angle.z);
                  ClientPrint (pEntity, HUD_PRINTNOTIFY, msg);

                  UTIL_TraceHull (pEntity->v.origin + pEntity->v.view_ofs,
                                  pEntity->v.origin + pEntity->v.view_ofs + gpGlobals->v_forward * atoi (arg1),
                                  ignore_monsters, head_hull, pContainingEntity, &tr);
                  UTIL_DrawBeam (pEntity, pEntity->v.origin + pEntity->v.view_ofs, pEntity->v.origin + pEntity->v.view_ofs + gpGlobals->v_forward * atoi (arg1) * tr.flFraction, 600, 10, 2, 250, 250, 250, 200, 10);

                  sprintf (msg, "distance traced: %.1f - Normal: vecPlaneNormal (%.1f, %.1f, %.1f)\n", atoi (arg1) * tr.flFraction, tr.vecPlaneNormal.x, tr.vecPlaneNormal.y, tr.vecPlaneNormal.z);
                  ClientPrint (pEntity, HUD_PRINTCONSOLE, msg);
               }
            }
            else
...
void StartFrame (void)
{
   ...
   if (is_multiplayer_game)
   {
      ...
      if (roundend)
      {
         ...
         b_bomb_planted = FALSE; // reset any 'bomb planted' signal
         ...
         if (bots[bot_index].is_active && (bots[bot_index].pEdict != NULL))
         {
                ...
                && (bots[bot_index].quit_game_time <= gpGlobals->time))
            {
               ...
               if (bot_check_time - 5.0 < gpGlobals->time)
                  bot_check_time = gpGlobals->time + RANDOM_FLOAT (20.0, 40.0); // and delay
            }
         }
      }
