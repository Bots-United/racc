...
int Spawn (edict_t *pent)
{
   ...
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      if (strcmp (STRING (pent->v.classname), "worldspawn") == 0)
         ...
         round.b_bomb_map = TRUE; // remember when it is a bomb map

      else if (strcmp (STRING (pent->v.classname), "hostage_entity") == 0)
      {
         round.b_hostage_map = TRUE; // remember when it is a hostage map
         round.hostage_count++; // there is one hostage more on this map
      }
...
void StartFrame (void)
{
   ...
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      ...
      if (round.has_finished)
      {
         ...
         round.f_start_time = *server.time + CVAR_GET_FLOAT ("mp_freezetime"); // start time
         round.b_bomb_planted = FALSE; // reset any 'bomb planted' signal
      ...
      for (bot_index = 0; bot_index < RACC_MAX_CLIENTS; bot_index++)
      {
             ...
             && (bots[bot_index].quit_game_time <= *server.time))
         {
            ...
            if ((server.bot_check_time > 0) && (server.bot_check_time - 5.0 < *server.time))
               server.bot_check_time = *server.time + RANDOM_FLOAT (20.0, 40.0); // and delay
         }
      }
