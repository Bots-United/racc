...
void BotInitPathMachine (bot_t *pBot)
{
   ...
   pathmachine = &pBot->BotBrain.PathMachine; // first get a quick access to the bot's pathmachine
...
void BotRunPathMachine (bot_t *pBot)
{
   ...
   pathmachine = &pBot->BotBrain.PathMachine; // quick access to pathmachine
   pathmemory = pBot->BotBrain.PathMemory; // quick access to pathmemory
   ...
   if (pathmachine->should_update_goal)
   {
      ...
      walkface_at_feet = players[ENTINDEX (pBot->pEdict) - 1].pFaceAtFeet;
   ...
   while (pathmachine->open_count > 0)
   {
      ...
      if (DebugLevel.navigation > 2)
      {
         ...
         UTIL_DrawWalkface (pListenserverEntity, examined_node->walkface, 25, 255, 255, 255);
      ...
      if (examined_node == goal_node)
      {
         ...
         // if we want to debug the navigation, print out the path
         if (DebugLevel.navigation > 0)
         {
            ...
            for (link_index = 0; link_index < pathmachine->path_count; link_index++)
            {
               ...
               if (DebugLevel.navigation > 1)
               {
                  if (link_index + 1 < pathmachine->path_count)
                     UTIL_DrawLine (pListenserverEntity, pathmachine->path[link_index]->v_origin, pathmachine->path[link_index + 1]->v_origin, 600, 0, 0, 255);
                  UTIL_DrawNavlink (pListenserverEntity, pathmachine->path[link_index], 600);
               }
            }
         }
      ...
      for (link_index = 0; link_index < examined_node->links_count; link_index++)
      {
         ...
         if (DebugLevel.navigation > 2)
         {
            ...
            UTIL_DrawWalkface (pListenserverEntity, ancestor_node->walkface, 25, 255, 0, 0);
         }

         // compute its travel cost by adding this of its parent to its own
         ancestor_travel_cost = examined_node->travel_cost + BotEstimateTravelCost (pBot, examined_node->entrypoint, &examined_node->links[link_index]);
...
bool BotFindPathTo (bot_t *pBot, Vector v_goal, bool urgent)
{
   ...
   startface = WalkfaceUnder (v_goal);
   ...
   startnode->remaining_cost = (v_goal - pBot->pEdict->v.origin).Length ();
...
bool BotFindPathFromTo (bot_t *pBot, Vector v_start, Vector v_goal, bool urgent)
{
   ...
   goalface = WalkfaceUnder (v_start);
...
float BotEstimateTravelCost (bot_t *pBot, navlink_t *link_from, navlink_t *link_to)
{
   ...
   if (link_to->reachability & REACHABILITY_LADDER)
      f_cost *= pBot->BotBrain.likelevel_ladder;
   if (link_to->reachability & REACHABILITY_FALLEDGE)
      f_cost *= pBot->BotBrain.likelevel_falledge;
   if (link_to->reachability & REACHABILITY_ELEVATOR)
      f_cost *= pBot->BotBrain.likelevel_elevator;
   if (link_to->reachability & REACHABILITY_PLATFORM)
      f_cost *= pBot->BotBrain.likelevel_platform;
   if (link_to->reachability & REACHABILITY_CONVEYOR)
      f_cost *= pBot->BotBrain.likelevel_conveyor;
   if (link_to->reachability & REACHABILITY_TRAIN)
      f_cost *= pBot->BotBrain.likelevel_train;
   if (link_to->reachability & REACHABILITY_LONGJUMP)
      f_cost *= pBot->BotBrain.likelevel_longjump;
