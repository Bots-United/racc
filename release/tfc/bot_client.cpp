...
void BotClient_TFC_VGUIMenu (void *p, int bot_index)
{
   ...
   if ((*(int *) p) == 2) // is it a team selection menu?
      bots[bot_index].BotEyes.BotHUD.menu_state = MENU_TFC_TEAMSELECT_TEAMMENU;

   else if ((*(int *) p) == 3) // is is a class selection menu?
      bots[bot_index].BotEyes.BotHUD.menu_state = MENU_TFC_TEAMSELECT_CLASSMENU;
}
...
{
   // this message tells a game client to raise or lower the amount of a certain type of his
   // player's grenades (there are two types of grenades in Team Fortress Classic).

   static int grenade_type;

   if (messagestate == 0)
      grenade_type = *(int *) p; // 0 = primary grenades, 1 = secondary grenades
   else if (messagestate == 1)
   {
      if (grenade_type == 0)
         bots[bot_index].bot_grenades_1 = *(int *) p; // update the amount of primary grenades
      else if (grenade_type == 1)
         bots[bot_index].bot_grenades_2 = *(int *) p; // update the amount of secondary grenades
   }
}
