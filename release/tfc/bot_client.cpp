...
void BotClient_TFC_VGUI (void *p, int bot_index)
{
   if ((*(int *) p) == 2) // is it a team select menu?
      bots[bot_index].menu_notify = MSG_TFC_TEAMSELECT_TEAMMENU;

   else if ((*(int *) p) == 3) // is is a class selection menu?
      bots[bot_index].menu_notify = MSG_TFC_TEAMSELECT_CLASSMENU;
}
...
void BotClient_TFC_SecAmmoVal (void *p, int bot_index)
{
   ...
   else if (state == 1)
   {
      ...
      if (type == 0)
         bots[bot_index].bot_grenades_1 = *(int *) p; // the amount of ammo currently available
      else if (type == 1)
         bots[bot_index].bot_grenades_2 = *(int *) p; // the amount of ammo currently available
   }
}
   ...
   else if (state == 2)
   {
      ...
      if (index != -1)
         if ((killer_index == 0) || (killer_index == victim_index))
            bots[index].pKillerEntity = NULL; // bot killed by worldspawn or by self...
         else
