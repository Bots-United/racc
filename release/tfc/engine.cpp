...
edict_t *pfnFindEntityByString (edict_t *pEdictStartSearchAfter, const char *pszField, const char *pszValue)
{
   ...
   if (server.is_multiplayer && !DebugLevel.is_paused)
      if (strcmp (pszValue, "info_map_parameters") == 0)
         round.has_finished = TRUE; // catch new round message (resetting map message)
...
void pfnEmitSound (edict_t *entity, int channel, const char *sample, float volume, float attenuation, int fFlags, int pitch)
{
   ...
   // FIXME: move that old relic that's commented below to BotReactToSound()
/*   // only process bots if we are in multiplayer mode
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      int index;
      ...
      if ((strcmp (sample, "speech/saveme1.wav") == 0) || (strcmp (sample, "speech/saveme2.wav") == 0))
      {
         ...
         for (index = 0; index < RACC_MAX_CLIENTS; index++)
         {
            ...
            if ((BotGetIdealAimVector (&bots[index], entity) != g_vecZero)
                && FNullEnt (bots[index].pEnemy)
                && ((bots[index].pEdict->v.origin - entity->v.origin).Length () < 1000))
            {
               bots[index].pEnemy = entity; // go and heal this player
   ...
   if (server.is_multiplayer && !DebugLevel.is_paused)
   {
      ...
      if (IsValidPlayer (ed))
      {
         ...
         if (ed->v.flags & FL_THIRDPARTYBOT)
         {
            ...
            if (msg_type == GetUserMsgId ("VGUIMenu"))
               ...
               botMsgFunction = BotClient_TFC_WeapPickup;
      ...
      else if (msg_dest == MSG_ALL)
      {
         ...
         if (msg_type == GetUserMsgId ("DeathMsg"))
            botMsgFunction = BotClient_TFC_DeathMsg_All;
      }
   }
